...
- **`ExecutionResult.failure_category`** — a safe accessor for *why* a call isn't ok: returns the validation category or `"executor_error"`. `result.failure` is set only for validation failures and `result.error` for executor exceptions, so a bare `result.failure.category` could `AttributeError` when the tool's own code raised. The README example now uses `failure_category` / `raise_on_failure()`.
